...
class TestEventType(unittest.TestCase):
    ...
    def test_init(self):
        ...
        eventtype = detectionformats.eventtype.EventType(self.TYPE, self.CERTAINTY)
    ...
    def test_ValidTypes(self):
        eventtype = detectionformats.eventtype.EventType(
            self.EARTHQUAKE, self.CERTAINTY
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(
            self.MINECOLLAPSE, self.CERTAINTY
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(
            self.NUCLEAREXPLOSION, self.CERTAINTY
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(
            self.QUARRYBLAST, self.CERTAINTY
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(self.INDUCED, self.CERTAINTY)
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(self.ROCKBURST, self.CERTAINTY)
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(
            self.FLUIDINJECTION, self.CERTAINTY
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(self.ICEQUAKE, self.CERTAINTY)
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(self.VOLCANIC, self.CERTAINTY)
        self.assertTrue(eventtype.isValid())

    def test_ValidCertainties(self):
        eventtype = detectionformats.eventtype.EventType(
            self.EARTHQUAKE, self.SUSPECTED
        )
        self.assertTrue(eventtype.isValid())

        eventtype = detectionformats.eventtype.EventType(
            self.EARTHQUAKE, self.CONFIRMED
        )
        self.assertTrue(eventtype.isValid())
