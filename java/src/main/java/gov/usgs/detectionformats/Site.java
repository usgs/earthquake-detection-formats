...
/**
 * a conversion class used to create, parse, and validate site data as part of detection data.
 *
...
public class Site implements DetectionInt {
  ...
  /**
   * The advanced constructor for the Site class. Initializes members to provided values.
   *
   ...
   */
  public Site(String newStation, String newChannel, String newNetwork, String newLocation) {
    station = newStation;
    channel = newChannel;
    network = newNetwork;
   ...
   * @param newNetwork - A String containing the network to use
   * @param newLocation - A String containing the location to use (null omit)
   * @param newLatitude - A Double containing the latitude in degrees to use (null omit)
   * @param newLongitude - A Double containing the longitude in degrees to use (null omit)
   * @param newElevation - A Double containing the elevation in meters to use (null omit)
   */
  public Site(
      String newStation,
      String newChannel,
  ...
  public ArrayList<String> getErrors() {
    ...
    if (jsonLatitude != null) {
      if ((jsonLatitude < -90.0) || (jsonLatitude > 90.0)) {
        // invalid latitude
        errorList.add("Latitude in Site Class not in the range of -90 to 90 degrees.");
      }
    }
    ...
    if (jsonLongitude != null) {
      if ((jsonLongitude < -180.0) || (jsonLongitude > 180.0)) {
        // invalid longitude
        errorList.add("Longitude in Site Class not in the range of -180 to 180 degrees.");
      }
    }
    ...
    if (jsonElevation != null) {
      if ((jsonElevation < -500.0) || (jsonElevation > 8900.0)) {
        // invalid elevation
        errorList.add("Elevation in Site Class not in the range of -500 to 8900 meters.");
      }
    }
