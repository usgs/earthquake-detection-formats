...
namespace detectionformats
{
	...
	*
	* The detectionformats origin class is a conversion class used to create, parse, and
	* validate the event origin data format origin.  The origin format uses
	...
	*/
	...
	public:
		...
		* \param newtime - A double containting the new origin time to use
		* \param newdepth - A double containing the depth to use
		* \param neworigintype - A std::string containing the origin type to use, empty string to omit
		* \param neweventtype - A std::string containing the event type to use, empty string to omit
		* \param newbayes - A double containing the bayes to use, std::numeric_limits<double>::quiet_NaN() to omit
		* \param newminimumdistance - A double containing the minimum distance to use, std::numeric_limits<double>::quiet_NaN() to omit
		* \param newrms - A double containing the rms to use, -10000 to omit
		* \param newgap - A double containing the gap to use, std::numeric_limits<double>::quiet_NaN() to omit
		* \param newpickdata - A std::vector<detectionformats::pickjson> newdata containing the data that went into this origin, empty vector to omit
		* \param newbeamdata - A std::vector<detectionformats::beamjson> newdata containing the data that went into this origin, empty vector to omit
		* \param newcorrelationdata - A std::vector<detectionformats::correlationjson> newdata containing the data that went into this origin, empty vector to omit
		*/
		origin(std::string newid, std::string newagencyid, std::string newauthor, double newlatitude, double newlongitude, double newtime, double newdepth,
			          std::string neworigintype, std::string neweventtype, double newbayes, double newminimumdistance, double newrms, double newgap,
			          std::vector<detectionformats::pick> newpickdata, std::vector<detectionformats::beam> newbeamdata, std::vector<detectionformats::correlation> newcorrelationdata);
		...
		* \param newcorrelationdata - A std::vector<detectionformats::correlationjson> newdata containing the data that went into this origin, empty vector to omit
		*/
		origin(std::string newid, detectionformats::source newsource, double newlatitude, double newlongitude, double newtime, double newdepth,
		       	      std::string neworigintype, std::string neweventtype, double newbayes, double newminimumdistance, double newrms, double newgap,
			          std::vector<detectionformats::pick> newpickdata, std::vector<detectionformats::beam> newbeamdata, std::vector<detectionformats::correlation> newcorrelationdata);
		...
		*
		* A required double defining the latitude of this origin
		*/
		double latitude;
		...
		*
		* A required double defining the longitude of this origin
		*/
		double longitude;
		...
		* \brief origin time
		*
		* A required double containing the time for this origin
		*/
		double time;
		...
		* \brief depth value
		*
		* A required double defining the detph of this origin
		*/
		double depth;
