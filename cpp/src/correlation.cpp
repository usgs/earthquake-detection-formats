...
#define ORIGINTIME_KEY "OriginTime"
...
namespace detectionformats
{
	correlation::correlation()
	{
		...
		origintime = std::numeric_limits<double>::quiet_NaN();
		eventtype = "";
	...
	correlation::correlation(std::string newid, std::string newsiteid, std::string newstation, std::string newchannel, std::string newnetwork, std::string newlocation, std::string newagencyid, std::string newauthor, std::string newphase, double newtime, double newcorrelation, double newlatitude, double newlongitude, double newdepth, double neworigintime, std::string neweventtype, double newmagnitude, double newsnr, double newzscore, double newdetectionthreshold, std::string newthresholdtype)
	{
		type = CORRELATION_TYPE;
		id = newid;
	...
	correlation::correlation(std::string newid, std::string newsiteid, std::string newstation, std::string newchannel, std::string newnetwork, std::string newlocation, std::string newagencyid, std::string newauthor, std::string newphase, double newtime, double newcorrelation, double newlatitude, double newlongitude, double newdepth, double neworigintime, std::string neweventtype, double newmagnitude, double newsnr, double newzscore, double newdetectionthreshold, std::string newthresholdtype, std::string newassociatedphase, double newassociateddistance, double newassociatedazimuth, double newassociatedresidual, double newassociatedsigma)
	{
		type = CORRELATION_TYPE;
		id = newid;
		site = detectionformats::site(newsiteid, newstation, newchannel, newnetwork, newlocation);
	...
	correlation::correlation(std::string newid, detectionformats::site newsite, detectionformats::source newsource, std::string newphase, double newtime, double newcorrelation, double newlatitude, double newlongitude, double newdepth, double neworigintime, std::string neweventtype, double newmagnitude, double newsnr, double newzscore, double newdetectionthreshold, std::string newthresholdtype)
	{
		type = CORRELATION_TYPE;
		id = newid;
		site = newsite;
	...
	correlation::correlation(std::string newid, detectionformats::site newsite, detectionformats::source newsource, std::string newphase, double newtime, double newcorrelation, double newlatitude, double newlongitude, double newdepth, double neworigintime, std::string neweventtype, double newmagnitude, double newsnr, double newzscore, double newdetectionthreshold, std::string newthresholdtype, detectionformats::associated newassociated)
	{
		type = CORRELATION_TYPE;
		id = newid;
		site = newsite;
	...
	correlation::correlation(rapidjson::Value &json)
	{
		...
		// correlation
		if ((json.HasMember(CORRELATION_KEY) == true) && (json[CORRELATION_KEY].IsNumber() == true) && (json[CORRELATION_KEY].IsDouble() == true))
		...
		// origintime
		if ((json.HasMember(ORIGINTIME_KEY) == true) && (json[ORIGINTIME_KEY].IsString() == true))
			origintime = detectionformats::ConvertISO8601ToEpochTime(std::string(json[ORIGINTIME_KEY].GetString(), json[ORIGINTIME_KEY].GetStringLength()));
		else
			origintime = std::numeric_limits<double>::quiet_NaN();
		...
		// magnitude
		if ((json.HasMember(MAGNITUDE_KEY) == true) && (json[MAGNITUDE_KEY].IsNumber() == true) && (json[MAGNITUDE_KEY].IsDouble() == true))
		...
		// zscore
		if ((json.HasMember(ZSCORE_KEY) == true) && (json[ZSCORE_KEY].IsNumber() == true) && (json[ZSCORE_KEY].IsDouble() == true))
			zscore = json[ZSCORE_KEY].GetDouble();
		else
			zscore = std::numeric_limits<double>::quiet_NaN();

		// detectionthreshold
		if ((json.HasMember(DETECTIONTHRESHOLD_KEY) == true) && (json[DETECTIONTHRESHOLD_KEY].IsNumber() == true) && (json[DETECTIONTHRESHOLD_KEY].IsDouble() == true))
			detectionthreshold = json[DETECTIONTHRESHOLD_KEY].GetDouble();
		else
			detectionthreshold = std::numeric_limits<double>::quiet_NaN();

		// thresholdtype
		if ((json.HasMember(THRESHOLDTYPE_KEY) == true) && (json[THRESHOLDTYPE_KEY].IsString() == true))
			thresholdtype = std::string(json[THRESHOLDTYPE_KEY].GetString(), json[THRESHOLDTYPE_KEY].GetStringLength());
		else
			thresholdtype = "";
	...
	correlation::correlation(const correlation &newcorrelation)
	{
		...
		latitude = newcorrelation.latitude;
		longitude = newcorrelation.longitude;
		depth = newcorrelation.depth;
		origintime = newcorrelation.origintime;
	...
	rapidjson::Value & correlation::tojson(rapidjson::Value &json, rapidjson::MemoryPoolAllocator<rapidjson::CrtAllocator> &allocator)
	{
		json.SetObject();
		...
		// origintime
		if (std::isnan(origintime) != true)
		{
			std::string timestring = detectionformats::ConvertEpochTimeToISO8601(origintime);
			rapidjson::Value origintimevalue;
			origintimevalue.SetString(rapidjson::StringRef(timestring.c_str()), allocator);
			json.AddMember(ORIGINTIME_KEY, origintimevalue, allocator);
		}
		...
		if (thresholdtype != "")
		{
			...
			thresholdtypevalue.SetString(rapidjson::StringRef(thresholdtype.c_str()), allocator);
	...
	std::vector<std::string> correlation::geterrors()
	{
		...
		{
			// site not found
			errorlist.push_back("Site object did not validate in correlation class.");
		}
		...
		{
			// bad source
			errorlist.push_back("Source object did not validate in correlation class.");
		}
		...
		else
		{
			try
			{
				...
				{
					errorlist.push_back("Time did not validate in correlation class.");
				}
			}
		...
		{
			// latitude not found
			errorlist.push_back("No Latitude in correlation class.");
		}
		if ((latitude < -90) || (latitude > 90))
		{
			errorlist.push_back("Invalid Latitude in correlation class.");
		}
		...
		{
			// longitude not found
			errorlist.push_back("No Longitude in correlation class.");
		}
		if ((longitude < -180) || (longitude > 180))
		{
			errorlist.push_back("Invalid Longitude in correlation class.");
		}
		...
		{
			// depth not found
			errorlist.push_back("No Depth in correlation class.");
		}
		if ((depth < -100) || (depth > 1500))
		{
			errorlist.push_back("Invalid Depth in correlation class.");
		}

		// origintime
		if (std::isnan(origintime) == true)
		{
			errorlist.push_back("OriginTime is missing in correlation class.");
		}
		else
		{
			try
			{
				if (detectionformats::IsStringISO8601(detectionformats::ConvertEpochTimeToISO8601(origintime)) == false)
				{
					errorlist.push_back("OriginTime did not validate in correlation class.");
				}
			}
			catch (const std::exception & e)
			{
		...
		if (std::isnan(magnitude) != true)
		{
			...
			{
				errorlist.push_back("Invalid Magnitude in corrleation class.");
			}
		}
		...
		if (associationinfo.isempty() == false)
		{
			...
			{
				// site not found
				errorlist.push_back("AssociationInfo object did not validate in correlation class.");
			}
		}

		// since thresholdtype is a free text string, and snr, zscore, and detectionthreshold are numaric values
		// no further validation is required. 
		// NOTE: Further validation COULD be done to confirm that thresholdtype is valid, and that snr, zscore, 
		// and detectionthreshold were within expected bounds.
